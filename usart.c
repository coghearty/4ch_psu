...
}void usart_process_command(){	switch(command_in[0]){		case 't':			if(command_in[1] == '?'){				print_current_time();			}else if(command_in[2] == '='){				uint8_t tmp=0;				tmp = parse_assignment();				switch(command_in[1]){					case 'd':						if(tmp > 6){dayofweek_count = 6;}else{dayofweek_count=tmp;}						break;					case 'h':						if(tmp > 23){hours_count = 23;}else{hours_count=tmp;}						break;					case 'm':						if(tmp > 59){minutes_count = 59;}else{minutes_count=tmp;}						break;					default:						//do nothing					break;				}			}			break;				case 'w':			if(command_in[1] == '?'){				print_wakeup_time();				}else if(command_in[2] == '='){				uint8_t tmp=0;				tmp = parse_assignment();				switch(command_in[1]){					/*case 'd':					if(tmp > 6){dayofweek_count = 6;}else{dayofweek_count=tmp;}					break;*/					case 'h':					if(tmp > 23){wakeup_h = 23;}else{wakeup_h=tmp;}					break;					case 'm':					if(tmp > 59){wakeup_m = 59;}else{wakeup_m=tmp;}					break;					default:					//do nothing					break;				}			}			break;		default:			puts("Command not recognised\n");		break;	}}